...
var animation = {
    ...
    initialize: function() {
        ...
        this.context.drawImage(this.images[this.counter], 0, 0, this.canvas.width, this.canvas.width*this.images[this.counter].height/this.images[this.counter].width);
    ...
    mousemove: function(e) {
        ...
        if (lastMove >= movePixelsPerFrame || lastMove <= -movePixelsPerFrame) {
            ...
            animation.context.drawImage(animation.images[animation.counter], 0, 0, animation.canvas.width, animation.canvas.width*animation.images[animation.counter].height/animation.images[animation.counter].width);
        }
        if (e.cancelable) { e.preventDefault(); }
        return false;
    },
